...
public class Colosseum {
     ...
     */
    static final int TOTAL_ATT_DEF_LEVEL = 50;
    ...
    public static Pokemon buildPokemon() {
        ...
        while (tempPokemon.hitPoints == 0) {
            System.out.print("How many hit points will it have? (1-50): ");
            if (myScan.hasNextInt()) {
                int num = myScan.nextInt();
                if (num > 0 && num <= MAX_HIT_POINTS) {
                    tempPokemon.hitPoints = num;
                } else {
                    System.out.println("Sorry. Hit points must be between 1 and 50.");
                    myScan = new Scanner(System.in);
                }
            } else {
                System.out.println("Sorry. You need to input an int.");
                myScan = new Scanner(System.in);
            }
        }

        System.out.println("Split fifty points between attack level and defense level");
        while (tempPokemon.attackLevel == 0) {
            System.out.print("Enter your attack level (1-49): ");
            if (myScan.hasNextInt()) {
                int num = myScan.nextInt();
                if (num > 0 && num < TOTAL_ATT_DEF_LEVEL) {
                    tempPokemon.attackLevel = num;
                } else {
        ...
        while (tempPokemon.defenseLevel == 0) {
            System.out.print("Enter your defense level (1-" + (TOTAL_ATT_DEF_LEVEL - tempPokemon.attackLevel) + "): ");
            if (myScan.hasNextInt()) {
                int num = myScan.nextInt();
                if (num > 0 && num <= TOTAL_ATT_DEF_LEVEL - tempPokemon.attackLevel) {
                    tempPokemon.defenseLevel = num;
                } else {
                    System.out.println("Sorry. Defense level must be between 1 and "
                            + (TOTAL_ATT_DEF_LEVEL - tempPokemon.attackLevel) + ".");
                    myScan = new Scanner(System.in);
                }
            } else {
                System.out.println("Sorry. You need to input an int.");
    ...
    public static void printWhoIsAhead() {
        if (firstPokemon.hitPoints > secondPokemon.hitPoints) {
            ...
            System.out.println("It's a dead heat!");
        }
    }
